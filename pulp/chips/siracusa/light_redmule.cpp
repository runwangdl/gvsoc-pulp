...
vp::IoReqStatus LightRedmule::req(vp::Block *__this, vp::IoReq *req)
{
    ...
    uint64_t offset = req->get_addr() - 0x10201000;
    ...
    if ((is_write == 0) && (offset == 32) && (_this->redmule_query == NULL) && (_this->state.get() == IDLE))
    {
        ...
        ************************/
        ...
        // Ensure compute_able and elem_size are properly set
        if (_this->compute_able == 4) {
            _this->elem_size = 4; // Explicitly ensure FP32 has correct element size
            _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmule] Using FP32 compute mode with elem_size: %d\n", _this->elem_size);
        }
        ...
        *************************/
        ...
        _this->timer_start      = _this->time.get_time();
        _this->cycle_start      = _this->clock.get_cycles();
        _this->compute_able     = _this->compute_able; // Preserve the compute mode
        _this->event_enqueue(_this->fsm_event, 1);
        ...
        *************************/
        ...
        switch (offset) {
            ...
            case 24:
                ...
                break;
            case 28:
                _this->compute_able = value;
                if (_this->compute_able == 4) {
                    _this->elem_size = 4; // FP32
                } else if (_this->compute_able < 4) {
                    _this->elem_size = 2; // FP16/INT16/UINT16
                } else {
                    _this->elem_size = 1; // INT8/UINT8/FP8
                }
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[LightRedmule] Set compute mode 0x%x, elem_size: %d\n", value, _this->elem_size);
                break;
            case 32:
